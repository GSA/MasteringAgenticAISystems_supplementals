...
**Notes.** The Videos column counts the videos shown under each chapter summary below, out of the unique direct links in [`Part_10_YoutubeVideos.md`]({{ site.repo_blob }}/videos/Part_10_YoutubeVideos.md) ("3 of 5"). A video is left out when its link is dead, embedding is disabled, or YouTube's title does not match the entry; see the [link check]({% link video-link-check.md %}). Chapters can also list search suggestions instead of links.
