...
available where they were written. Before announcing the site, check the first
...
| Page | Built from |
|---|---|
| `curriculum/part-NN.md` | Chapter IDs from the five `cert_mapping/*.csv` files and the section headings of `Study_Plan.md` (titles, hours, summaries); resources matched from `slides/`, `figures/`, `code_examples/`, `videos/`, and `quizzes_ver20JUN26.md` (rules below) |
...
| Videos | Parts 7–10 list 48, 23, 64, and 68 entries but only 3, 2, 6, and 3 direct links; the rest are search suggestions | The Videos page and Part pages show link counts, not entry counts |
